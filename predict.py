...
=================================================================
Model: Logistic Regression (retrained on v3 data, 2026-08-26,
       class_weight='balanced', 14 features, sanity accuracy 92.2%)
- predict_fall_risk(features)      -> 'LOW' / 'MEDIUM' / 'HIGH'
- explain_patient(features)        -> LIME explanation (why + which way)
- get_top3_features()              -> Top-3 risk factors (global)
...
Input features (14): sex ('F'/'M' or 0/1) + the original 10 + 3 fall-detail
(days_since_last_fall: None/'' = never fell; syncopal_fall; fall_cluster_30d).
"""
import os
import json
import numpy as np
...
# regenerated by ml/train.py (coefficient-based, frequency-weighted)
TOP_3_FEATURES = [
  {
  ...
  {
    "feature": "orthostatic_hypotension",
    "meaning": "Dizzy when standing (0/1)",
    "weight": 1.0672
  }
]
...
def get_top3_features() -> list:
    """Top-3 risk factors (global) for the API."""
...
def get_minimal_change(features: dict) -> list:
    ...
    Unchangeable features (sex/age/history) are excluded from suggestions.
    """
...
def recommend_intervention(features: dict) -> dict:
    ...
    Works for patients of ANY age (complements the rule-based 60-74 suggestions
    in the backend with a personalized counterfactual for everyone).
    """
    pred = predict_fall_risk(features)
    if pred != 'HIGH':
...
if __name__ == "__main__":
    ...
    for e in explain_patient(test):
        print(f"      {e}")
    print(f"\n[4] POSSIBLE CHANGES (ranked, changeable features only):")
